...
class UserController {
  ...
  async postRegister(req: Request, res: Response) {
    ...
    req
      .checkBody("password", messages.validation_errors.password)
      .isLength({ min: 5 });
    ...
    passport.authenticate("signup", async (err, user, info) => {
      try {
        ...
        req.flash("message", info.message);
        res.redirect(accountify(Routes.register));
      }
    })(req, res);
  }
  ...
  async forgotPassword(req: Request, res: Response) {
    ...
    if (user) {
      const token = await jwt.sign({ email: user.email }, config.jwtSecret, {
        expiresIn: "1h",
      });
  ...
  async resetPassword(req: Request, res: Response) {
    ...
    req
      .checkBody("password", "Password must be at least 5 characters long")
      .isLength({ min: 4 });
    req
      .checkBody("new_password", "Passwords do not match")
      .equals(new_password);
